...
<div v-if="visible" class="row my-3">
    <div class="col-md-2">
        <img src="/img/competitions.png" class="rounded-circle mx-auto align-content-center" style="width:100px;">
    </div>
    <div class="col-md-6">
        <hr class="hidden-sm-up">
        <h4 class="h4-responsive my-1 my-md-0">{{competition.title}}</h4>
        <p class="text-truncate my-0">{{competition.description}}</p>
    </div>
    <div class="col-md-4" v-if="isOnProfile">
        <div class="actions d-flex justify-content-md-end  justify-content-sm-center">
            ...
            <button class="btn btn-warning px-2" @click="deleteItem(competition._id)"><i class="material-icons responsive">close</i></button>
        </div>
    </div>
</div>    
</template>
...
export default{
    ...
    methods:{
        deleteItem: function(_id){
            ...
            Vue.axios.delete(_baseUrl + 'api/profile/competition/' + _id)
            .then(function(res){
                console.log(res);
                vm.visible = false;
            })
